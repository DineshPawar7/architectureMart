  ...
  return (
    <div className="max-w-xl mx-auto p-6 bg-white shadow-lg rounded-xl mt-10">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <input id="imageInput" type="file" multiple onChange={handleImageChange} accept="image/*" className="border p-2 rounded" required />
        <input id="pdfInput" type="file" onChange={handlePdfChange} accept="application/pdf" className="border p-2 rounded" required />

        <button type="submit" className="bg-blue-600 text-white py-2 rounded hover:bg-blue-700 transition duration-200">Upload</button>
      </form>
    </div>
  );
};
